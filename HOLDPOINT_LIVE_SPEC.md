...
- [x] P2-09 Configurable lifecycle hooks (framework, phase 1 — Claude end-to-end). Extended the schema so checks attach to `on:` hook points (`session_start`, `message_submit`, `before_tool`, `after_tool`, `session_end`, `before_done`) and gained an `inject` behavior (text/files/datetime) alongside `cmd`/`prompt`. The CLI `check --hook <event>` filters by hook; the Claude engine seeds context at session start / message submit and gates `before_tool`. The Checks editor is reorganized to the model (list grouped by hook; color-coded When-it-runs / What-it-does / triggers sections), and the Activity filter became a category-grouped dropdown. Cursor/Codex/Copilot per-hook parity is follow-up work.
- [x] P2-10 Codex + Cursor per-hook parity (phase 2), verified against current hook APIs. Codex injects context at `SessionStart`/`UserPromptSubmit` and gates `before_tool` (exit 2). Cursor injects at `sessionStart` and denies tools via `preToolUse` `permission: "deny"`; since Cursor's `beforeSubmitPrompt` can't inject context, per-message datetime/context is folded into `sessionStart` (and the prior no-op `beforeSubmitPrompt` injection removed). Both engines' generated scripts are covered by functional execution tests. Copilot per-hook parity remains the last follow-up.
- [x] P2-11 Copilot per-hook parity (phase 3 — all engines done). Researched CLI hooks vs the SDK extension and kept the extension (only it can inject at `userPromptSubmitted`, gate completion reliably via `task_complete`, and power the Live control bridge; CLI hooks are observational outside `preToolUse` and preview-buggy). Generalized `onSessionStart` / `onUserPromptSubmitted` / `onPreToolUse` to the per-hook inject/prompt model and added a `before_tool` cmd gate (`permissionDecision: deny`). All four engines now support session_start / message_submit / before_tool / before_done.

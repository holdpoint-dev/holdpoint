...
printf "  ${DIM}·${RESET} ${YELLOW}HOLDPOINT_PREREQUISITES.md${RESET}       agent setup notes and local prerequisites\n"
...
printf "  ${DIM}·${RESET} ${YELLOW}.cursor/hooks.json${RESET}                 Cursor hook\n"
printf "  ${DIM}·${RESET} ${YELLOW}.cursorrules${RESET}                       Cursor context\n"
...
printf "  ${DIM}3.${RESET} ${YELLOW}holdpoint check --staged${RESET} to run checks at any time\n"
printf "\n"
printf "  ${YELLOW}${BOLD}Copilot CLI note:${RESET} run ${YELLOW}/experimental on${RESET} so the ${YELLOW}EXTENSIONS${RESET} feature\n"
printf "  is enabled before using Holdpoint locally. See ${YELLOW}HOLDPOINT_PREREQUISITES.md${RESET}.\n"
printf "\n"
printf "  ${DIM}Visual builder · holdpoint builder${RESET}\n"

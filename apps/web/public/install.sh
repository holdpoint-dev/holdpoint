...
info "Running: npx holdpoint@latest init --stack=${DETECTED_STACK} --agent=${DETECTED_AGENT}"
echo ""

npx --yes holdpoint@latest init --stack="${DETECTED_STACK}" --agent="${DETECTED_AGENT}"

echo ""
success "Holdpoint active."
echo ""
echo -e "  Edit ${HOLDPOINT_YELLOW}checks.yaml${HOLDPOINT_RESET} to customise your eval checkpoints."
echo -e "  Run   ${HOLDPOINT_YELLOW}npx holdpoint check${HOLDPOINT_RESET} to validate at any time."
echo -e "  Open  ${HOLDPOINT_YELLOW}npx holdpoint build${HOLDPOINT_RESET} for the visual builder."
echo ""

import { buildSecurityScanScript, type HoldpointConfig } from "@holdpoint/types";
...
if (hook === "session_start") {
  ...
  const scanSource = typeof input.source === "string" ? input.source : "startup";
  const runScan = scanSource !== "resume" && scanSource !== "compact";
  if (runScan && cfg.security_scan !== false) {

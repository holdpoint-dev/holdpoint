...
function gatherHookContext(hook, isSessionStart) {
  const parts = [];
  let hasDatetime = false;
  const addDatetime = () => { if (!hasDatetime) { hasDatetime = true; parts.push(DATETIME_TEXT()); } };

  if (hook === "session_start") {
    // #8: inject user-configured context files FIRST so they survive truncation;
    // the auto security-scan banner is appended LAST below.
    ...
    // #3: the 8s synchronous dependency audit must run only for a genuine top-level
    // SessionStart, never for SubagentStart (both map to "session_start"). Defaults ON.
    if (isSessionStart && cfg.security_scan !== false) {
...
// #3: true top-level session start only — SubagentStart maps to the same internal
// "session_start" hook but must NOT trigger the audit-bearing security scan.
...
if (eventName === "SessionStart" || eventName === "SubagentStart" || eventName === "UserPromptSubmit") {
  const context = gatherHookContext(hook, isSessionStart);
  sendLiveEvent({
    ...input,

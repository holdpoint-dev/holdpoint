...
function runHoldpointChecks(repoRoot, command = ${JSON.stringify(checkCommand)}) {
  ...
  hooks: {
    ...
    onPreToolUse: async (input) => {
      ...
      if (hasCmdAt(repoRoot, 'before_tool')) {
        const result = runHoldpointChecks(repoRoot, ${JSON.stringify(checkCommand + " --hook before_tool")});
        if (!result.ok) {
